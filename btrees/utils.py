...
from BTrees._base import _TreeItem
...
def get_samples_csv():
    if os.path.isfile(common.SAMPLING_TESTS_CSV):
        df =  pd.read_csv(common.SAMPLING_TESTS_CSV)
    else:
        df = pd.DataFrame()
...
# node._data[0].child arrives to actual child,
# if it's a bucket

def iter_btree_level(node, level, so_far=[]):
    # wrong way to check the type, because of cyclic imports
    try:
        assert isinstance(node, node._bucket_type) or 'OOBTreeExt' in str(type(node))
    except:
        raise
    # when in a node:
    #print(f"iterate level={level} node:{node}")

    if level==0:
        if isinstance(node, node._bucket_type):
            print('return bucket')
            return node
        else:
            print('return node')
            node._data # stay consistent with this

    if isinstance(node, node._bucket_type):
        print(f'got to a bucket although it has arrive to level {level}')
        return node._data


    for pointer in node._data:
        #return list(map(lambda x: iter_btree_level(x, level-1), child.child))
        so_far.append(iter_btree_level(pointer.child, level-1, so_far))
    return so_far
