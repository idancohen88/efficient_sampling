...
class OOBTreeExtOlken(OOBTreeBase):
    def sample_olken_early_abort(self, k):
        ...
        self._persist_sampling_stats(reject_counter=reject_counter, start_time=start_time, sampled_values=sampled_values,
                                     name='olken_early_abort', sample_size=k, revisit_counter=revisit_counter)
    ...
    def sample_olken(self, k):
        ...
        while len(sampled_values) < k:
            ...
            self._accept_reject_probs.append(acc_rej_prob)
            
            if not _accept_reject_test_pass(acc_rej_prob):
                reject_counter += 1
                continue
        ...
        self._persist_sampling_stats(reject_counter=reject_counter, start_time=start_time, sampled_values=sampled_values,
                                     name='olken', sample_size=k, revisit_counter=revisit_counter)

        self.save_sampled_path(name='olken', k=k, sampled_paths=sampled_paths)
        return sampled_values
