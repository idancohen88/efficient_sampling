...
class OOBTreeExtFanoutOriented(OOBTreeBase):
    def __init__(self):
        ...
        self.root_probs_coefs = None
        self.root_coef_height_4_duration = None

    def sample_distribution_oriented_height_three(self, k):
        global FANOUT_DISTRIBUTION_CACHE
        FANOUT_DISTRIBUTION_CACHE = {}
        ...
        self._persist_sampling_stats(start_time=start_time,sampled_values=sampled_values,
                                     name='distribution_oriented_height_three', sample_size=k, revisit_counter=revisit_counter)
    ...
    def _fanout_oriented_random_walk_node_to_item(self, node):
        ...
        while not isinstance(current_node, self._bucket_type):
            ...
            current_node = current_node._data[next_random_step].child  #next_random_step=125
    ...
    def determine_root_coefs(self):
        self.root_probs_coefs = self._first_walk_to_determine_root_coefs()
    ...
    def sample_distribution_oriented_height_four(self, k):
        ...
        if self.root_probs_coefs is None:
            self.root_probs_coefs = self._first_walk_to_determine_root_coefs() # todo:

        start_time = datetime.now()
        root_probs_coefs = self.root_probs_coefs
        sampled_values = []
        sampled_paths = set()
        revisit_counter = 0
        ...
        while len(sampled_values) < k:
            ...
            if path in sampled_paths:
                revisit_counter += 1
                continue # todo: revisit counter

            sampled_values.append(value)
            sampled_paths.add(path)

        self._persist_sampling_stats(start_time=start_time, sampled_values=sampled_values,
                                     name='distribution_oriented_height_four', sample_size=k, revisit_counter=revisit_counter)
    ...
    def _first_walk_to_determine_root_coefs(self):
        ...
        x = np.linalg.solve(equations_matrix, equations_equal_matrix)
        end_time = datetime.now()
        duration = (end_time - start_time).seconds
        self.root_coef_height_4_duration = duration
        return x
    ...
    def _calc_fanout_distribution_of_node(self, node):
        if node in self._fanout_distribution_cache: #if node in FANOUT_DISTRIBUTION_CACHE:
            self._cache_hit_counter['hit'] += 1
            return self._fanout_distribution_cache[node] # return FANOUT_DISTRIBUTION_CACHE[node]
        ...
        self._fanout_distribution_cache[node] = node_distribution        # FANOUT_DISTRIBUTION_CACHE[node] = node_distribution
