...
BTREE_CODE_VERSION = "1.6._fix_revisit_bug_colab" # removed "copy" of distribution vectors
DUMMIES_SAMPLING_METHODS = ["sample_monkey", "sample_numpy", "sample_naive_random_walk"]


TEST_STATS_ITER_LOOPS=5
...
class OOBTreeBase(OOBTreePy):
    ...
    def _calculate_avg_ks_test(self, sampled_values, sample_size):
        all_ks_stats = []
        all_p_values = []
        for _ in range(TEST_STATS_ITER_LOOPS):
    ...
    def _persist_sampling_stats(self, **kwargs):
        ...
        revisit_counter = kwargs.get("revisit_counter", 0)
        ...
        anderson_darling = self._calc_anderson_darling_averaging_x_times(sampled_values, sample_size)
    ...
    def _calc_anderson_darling_averaging_x_times(self,sampled_values, sample_size):
        anderson_stats_list = []
        significant_level_list = []
        for i in range(5):
            anderson_stats, significant_level = self._calc_anderson_darling(sampled_values, sample_size, force_new_np_sample=True)
            anderson_stats_list.append(anderson_stats)
            significant_level_list.append(significant_level)

        return statistics.mean(anderson_stats_list), statistics.mean(significant_level_list)

    def _calc_anderson_darling(self, sampled_values, sample_size, force_new_np_sample=False):
    ...
    def _append_to_df(self, **kwargs):
        ...
        with lock:
            ...
            samples_df.to_csv(common.SAMPLING_TESTS_CSV, index=False)
